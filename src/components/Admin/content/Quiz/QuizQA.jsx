...
const QuizQA = () => {
  ...
  const fetchQuizWithQA = async () => {
    ...
    if (data && data.EC === 0) {
      ...
      let newQA = [];
      for (const element of data.DT.qa) {
        if (element.imageFile) {
          element.imageName = `Question-${element.id}.png`;
          element.imageFile = await urltoFile(
            `data:image/*;base64,${element.imageFile}`,
            `Question-${element.id}.png`,
            "image/*"
          );
        }
        newQA.push(element);
  ...
  return (
    <div className="manage-question-container">
      <div className="add-new-question mt-3">
        ...
        <div className="my-3">Update questions</div>
        {questions &&
          questions.length > 0 &&
        ...
        {questions.length === 0 && (
          <div className="mb-3">
            <b>There are no questions in this quiz</b>
          </div>
        )}
      </div>
      {isPreviewImage && (
